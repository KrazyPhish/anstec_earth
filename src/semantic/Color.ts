...
export class Color {
   ...
   */
  toCzmColor() {
    return Czm_Color.fromBytes(this.red, this.green, this.blue, this.alpha)
  }
  ...
  toCssColorString() {
    ...
    } else return `rgba(${this.red}, ${this.green}, ${this.blue}, ${this.alpha / 255.0})`
  }
  ...
  static fromCssColorString(@is(String) color: string, @is(Color) result?: Color) {
    ...
    color = color.trim()
    matches = rgbaMatcher.exec(color)
    if (matches !== null) {
    ...
    matches = rgbFuncMatcher.exec(color)
    if (matches !== null) {
    ...
    matches = hslFuncMatcher.exec(color)
    if (matches !== null) {
      const [h, s, l, a] = [
