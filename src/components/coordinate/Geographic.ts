...
import { validate, moreThan, is, mutipleOf, lessThan, positive, constant } from "decorators"
...
export class Geographic {
   ...
   */
  @validate
  static fromDegreesArray(@mutipleOf(2) @is(Array) coordinates: number[]) {
   ...
   */
  @validate
  static fromRadiansArray(@mutipleOf(2) @is(Array) coordinates: number[]) {
    const geographics: Geographic[] = []
    for (let i = 0; i < coordinates.length; i += 2) {
      const lon = coordinates[i]
      const lat = coordinates[i + 1]
   ...
   */
  @validate
  static fromDegreesArrayHeights(@mutipleOf(3) @is(Array) coordinates: number[]) {
    const geographics: Geographic[] = []
   ...
   */
  @validate
  static fromRadiansArrayHeights(@mutipleOf(3) @is(Array) coordinates: number[]) {
    const geographics: Geographic[] = []
    for (let i = 0; i < coordinates.length; i += 3) {
      const lon = coordinates[i]
      const lat = coordinates[i + 1]
